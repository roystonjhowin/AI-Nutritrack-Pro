...
/* Shared page context — the AI coach reads this so it can answer
   using the numbers the visitor has already worked out here. */
window.NT = { diet: null, bmi: null, food: { items: [], totals: {} } };
...
calcForm.addEventListener('submit', (e) => {
  ...
  window.NT.diet = {
    gender, age, heightCm: height, weightKg: weight,
    activityFactor: activity, goal: goalLabel,
    bmr: Math.round(bmr), tdee: Math.round(tdee),
    calorieTarget: Math.round(target),
    proteinG: Math.round(proteinG), carbG: Math.round(carbG), fatG: Math.round(fatG),
    waterL: Number(water), fibreG: fiber
  };
...
async function searchWorldwideFood(foodName, qty = 100, unit = "g"){
  try {
    ...
    const item = {
      ...
      qty: `${qty} ${unit}`,
      calories: Number(calories), proteinG: Number(protein),
      carbG: Number(carbs), fatG: Number(fat)
    };
    window.NT.food.items.push(item);
    ...
    row.querySelector(".remove-btn").addEventListener("click", () => {
      const i = window.NT.food.items.indexOf(item);
      if (i > -1) window.NT.food.items.splice(i, 1);
...
if (bmiForm) {
  bmiForm.addEventListener("submit", (e) => {
    ...
    window.NT.bmi = { value: Number(bmi.toFixed(1)), status, heightCm: h, weightKg: w };
